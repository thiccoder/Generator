...
namespace Generator
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            string[] dotxFiles = Directory.GetFiles(Environment.CurrentDirectory, "*.dotx");
            DocumentSelectBox.Items.AddRange(dotxFiles);
            string[] xmlFiles = Directory.GetFiles(Environment.CurrentDirectory, "*.xml");
            XmlSelectBox.Items.AddRange(xmlFiles);
         }
        ...
        private void OpenFile(ComboBox box)
        {
            bool xmlRead = false;
            bool dotxRead = false;
            while (!(xmlRead && dotxRead))
            {
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    string fileName = box.SelectedItem.ToString(); ;
                    if (!xmlRead)
                    {
                        CurrentXmlFile = fileName;
                        ReadXML(fileName);
